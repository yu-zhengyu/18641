package model;
import java.io.Serializable;
import java.util.ArrayList;

import model.OptionSet.Option;
/**
 * @version  1.0
 * @author YuZheng
 * @Date 9/14/2015
 * 
 * This class is a test file, which would create a Automotive class,
 * is implements the Serializable interface. Provide many functions to deal witt 
 * the option set and option. ALL CRUD functions are provided.
 */ 

public class Automobile implements Serializable {
	private static final long serialVersionUID = -2647584167364070782L;
	private String make;
	private String name;
	private String model;
	private float baseprice;
	private ArrayList<OptionSet> opset;
	private ArrayList<Option> choice;
	
	// Construction
	
	
	public Automobile() {}

	public Automobile(String make, String model, float baseprice) {
		this.make = make;
		this.name = make + " " + model;
		this.model = model;
		this.baseprice = baseprice;
		this.opset = new ArrayList<OptionSet>();
		this.choice = new ArrayList<Option>();
	}

	// all getter and setter generated by IDE
	public synchronized String getModel() {
		return model;
	}

	public synchronized void setModel(String model) {
		this.model = model;
	}

	public synchronized String getMake() {
		return make;
	}


	public synchronized void setMake(String make) {
		this.make = make;
	}


	public synchronized ArrayList<Option> getChoice() {
		return choice;
	}


	public synchronized void setChoice(ArrayList<Option> choice) {
		this.choice = choice;
	}


	public synchronized void setOpset(ArrayList<OptionSet> opset) {
		this.opset = opset;
	}
	
	public synchronized String getName() {
		return name;
	}
	
	public synchronized void setName(String name) {
		this.name = name;
	}
	
	public synchronized float getBaseprice() {
		return baseprice;
	}
	
	public synchronized void setBaseprice(float baseprice) {
		this.baseprice = baseprice;
	}
	
	
	public synchronized ArrayList<OptionSet> getAllOpset() {
		return opset;
	}
	
	public synchronized int getOptionSize(int index) {
		OptionSet o = getOpset(index);
		return o.getOpt().size();
	}
	
	public synchronized String getOptionSetName(int index) {
		OptionSet o = getOpset(index);
		return o.getName();
	}
	
	public synchronized String getOptionName(int opsetIndex, int opindex) {
		OptionSet o = getOpset(opsetIndex);
		return o.getOption(opindex).getName();
		
	}

	/** 
	 * Additional getters and setters
	 */
	
	// new function for project1 unit 5
	public synchronized String[] getopsetname() {
		ArrayList<OptionSet> temp = getAllOpset();
		String[] setnamelist = new String[getOpsetSize()];
		for(int i = 0; i < setnamelist.length; i++) {
			setnamelist[i] = temp.get(i).getName();
		}
		return setnamelist;
		
	}
	
	public synchronized int getOpsetSize() {
		return opset.size();
	}
	
	public synchronized String[] getOptionName(String opsetName) {
		OptionSet tempset = getOpset(opsetName);
		ArrayList<Option> tempop = tempset.getOpt();
		String[] opnamelist = new String[tempop.size()];
		for(int i = 0; i < tempop.size(); i++) {
			opnamelist[i] = tempop.get(i).getName();
		}
		return opnamelist;
	}
	
	public synchronized float getOptionPrice(String setName, String opname) {
		Option o = findOption(setName, opname);
		return o.getPrice();
	}
	
	// find or get option set
	public synchronized OptionSet getOpset (int index) {
		if (index > opset.size() || index < 0 || opset.get(index) == null)
			return null;
		else
			return opset.get(index);
	}
	public synchronized OptionSet getOpset (String name) {
		for (int i = 0; i < opset.size(); i++) {
			if (opset.get(i).getName().equals(name))
				return opset.get(i);
		}
		return null;
	}
	
	// set option set
	public synchronized void setOpset (OptionSet opset, int index) {
		this.opset.add(opset);
	}
	public synchronized void setOpset (String name) {
		opset.add(new OptionSet(name));
	}
	
	// find option set by name
	public synchronized OptionSet findOpset (String name) {
		for (int i = 0; i < opset.size(); i++) {
			if (opset.get(i) != null) {
				if (opset.get(i).getName().equals(name)) 
					return opset.get(i);
			}
		}
		return null;
	}
	
	// Update Option Set
	public synchronized void updateOptionSet (String oldname, Option opt) {
		for (int i = 0; i < opset.size(); i++) {
			if (opset.get(i).getName().equals(oldname)) {
				opset.get(i).setOption(opt);
			}
		}
	}
	// Update Option set name only
	public synchronized void updateOptionSetNmae (int index, String name) {
		if (index < 0 || index > opset.size())
			return;
		opset.get(index).setName(name);
	}
	public synchronized void updateOptionSetNmae (String oldname, String name) {
		if (findOpset(oldname) != null)
			findOpset(oldname).setName(name);
	}
	
	
	// Delete the option set using its index
	public synchronized void deleteOptionSet (int index) {
		if (index < 0 || index > opset.size())
			return;
		if (opset.get(index) != null)
			opset.remove(index);
	}
	
	// delete the option set using its name
	public synchronized void deleteOptionSet (String name) {
		for (int i = 0; i < opset.size(); i++) {
			if (opset.get(i).getName().equals(name)) {
				opset.remove(i);
			}
		}
	}
	
	// get option
	public synchronized Option findOption (int index, String opname) {
		if (index >= 0 && index < opset.size()) {
			return opset.get(index).getOption(opname);
		}
		return null;
	}
	
	// find option by option name
	public synchronized Option findOption (String opsetname, String opname) {
		for (int i = 0; i < opset.size(); i++) {
			if (opset.get(i).getName().equals(opsetname)) {
				return opset.get(i).getOption(opname);
			}
		}
		return null;
	}
	
	// set option
	public synchronized void setOption (int index, String name, float price) {
		if (getOpset(index) != null) {
			getOpset(index).setOption(name, price);
		}
	}
	
	public synchronized void setOption(String setName, String name, float price) {
		if (findOpset(setName) != null) {
			findOpset(setName).setOption(name, price);
		}
	}
	 
	public synchronized void setOption(int opsetindex, int opindex, String name, float price) {
		if(opsetindex >= 0 && opsetindex < opset.size()) {
			opset.get(opindex).setOption(name, price);
		}
	}
	
	// delete option using the index
	public synchronized void deleteOption (int index, String opname) {
		if (index == 0 && index < opset.size()) {
			if (opset.get(index) != null)
				opset.get(index).deleteOption(opname);
		}
	}
	// delete option using the option set name
	public synchronized void deleteOption (String opsetname, String opname) {
		for (int i = 0; i < opset.size(); i++) {
			if (opset.get(i).getName().equals(opsetname)) {
				opset.get(i).deleteOption(opname);
			}
		}
	}
	
	// Update option name only by using the index
	public synchronized void updateOptionName (int index, String oldname, String name) {
		if (index >= 0 && index < opset.size()) {
			opset.get(index).updateOptionName(oldname, name);
		}
	}
	// Update option name only by using the opiton set name
	public synchronized void updateOptionName (String opsetname, String oldname, String name) {
		if (findOpset(opsetname) != null)
			findOpset(opsetname).updateOptionName(oldname, name);
	}
	
	// Update option price only
	public synchronized void updateOptionPrice (int index, String name, float price) {
		if (index >= 0 && index < opset.size()) {
			opset.get(index).updateOptionprice(name, price);
		}
	}
	// update the option price 
	public synchronized void updateOptionPrice (String opsetname, String name, float price) {
		if (findOpset(opsetname) != null)
			findOpset(opsetname).updateOptionprice(name, price);
	}
	
	
	// Print Information
	public synchronized void printBasicInfomation () {
		System.out.println("The model name is: " + getName());
		System.out.println("The basic price is: " + getBaseprice());
		System.out.println();
	}
	
	// print the detail information about the option
	public synchronized void printoption(int index) {
		int i = index;
		if (opset.get(i) != null) {
			System.out.println(opset.get(i).getName());
			System.out.println("--------------------");
			opset.get(i).printoption();
			System.out.println();
		}
	}
	public synchronized void printoption() {
		for (int i = 0; i < opset.size(); i++) {
			if (opset.get(i) != null) {
				System.out.println(opset.get(i).getName());
				System.out.println("--------------------");
				opset.get(i).printoption();
				System.out.println();
			}
		}
	}
	
	// deal with choice
	public synchronized String getOptionChoice(String opsetName) {
		return getOpset(opsetName).getChoiceName();
	}
	public synchronized float getOptionChoicePrice(String opsetName) {
		return getOpset(opsetName).getChoicePrice();
	} 
	public synchronized void setOptionChoice(String opsetname, String opname) {
		OptionSet opset = getOpset(opsetname);
		choice.add(opset.getOption(opname));
		opset.setChoice(opname);
	}
	// computing the total price
	public synchronized float getTotalPrice() {
		float sum = baseprice;
		for (Option op : choice) {
			sum += op.getPrice();
		}
		return sum;
	}
	// pint total price
	public synchronized void printTotalPrice() {
		System.out.println("The total price is: " + getTotalPrice());
	}
	
	// pint choice
	public synchronized void printChoice() {
		System.out.println("Here are choices the user selected:");
		int i = 0;
		for (Option op : choice) {
			System.out.println(opset.get(i++).getName() + ": " + op.getName() + "\t"
								+ "Price: " + op.getPrice());
		}
	}
	
}
